const ICON_PATHS = {
  ...
  eyeOff:'<path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.53 13.53 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/><path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/><line x1="2" x2="22" y1="2" y2="22"/>',
...
function icon(name, opts = {}){
  ...
  const strokeAttr = opts.stroke ? ` stroke="${opts.stroke}"` : '';
  const sw = opts.strokeWidth ? ` stroke-width="${opts.strokeWidth}"` : ' stroke-width="2"';
  return `<svg class="${cls}"${sizeAttr} viewBox="0 0 24 24" fill="none"${strokeAttr}${sw} stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${body}</svg>`;
}
